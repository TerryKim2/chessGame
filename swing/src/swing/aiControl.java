...
public class aiControl {
	...
	/*A method of receiving information from a chess board, 
	 * sending information to methods existing in acontrol, 
	 * and moving AI's chess pieces based on this.*/
	public void controller(ChessBoard board, MoveGenerator mv, ChessPiece[][] bd) {
		...
		if (bestMove1.isEmpty() != true) {
			board.aiMovePiece(st, bestMove1.get(0).x, bestMove1.get(0).y);
		} else {
			board.aiMovePiece(st, bestMove1.get(0).x, bestMove1.get(0).y);
		}
	...
	// How to find the highest score. It's also added to prevent the selected piece from being unable to move.
	...
	//Scores are given according to the type of piece that exists in the movable compartment.
